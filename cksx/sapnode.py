...
def app_meta():
    ...
    with SAP_cnxn(**cnxn_details) as cnxn:
        ...
        resp = json.dumps({'data': data, 'columns': columns})
    return Response(resp)


def meta(cnxn_details, table_name, fields=None, sap_buffer_size=400):
    with SAP_cnxn(**cnxn_details) as cnxn:
        meta_result = cnxn.call('BBP_RFC_READ_TABLE',
                                QUERY_TABLE='DD03L',
                                DELIMITER='|',
    ...
    try:
        meta.drop('.INCLUDE', inplace=True)
    except ValueError:
        pass
    ...
    # Return
    resp = json.dumps({'data': data, 'columns': columns, 'vchunks': vchunks})
    return Response(resp)
